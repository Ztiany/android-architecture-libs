    ...
    private void adjustPositionForSuitingKeyboard() {
        if (null != mDecorView && null != mContentView) {
            ...
            popContentView.post(() -> {
                ...
                Util.log("editTextBottom = " + editTextBottom);

                popContentView.getLocationOnScreen(pos);
                int popBottom = pos[1];
                Util.log("popContentView y = " + pos[1]);

                mDifference = editTextBottom - popBottom;
                Util.log("mDifference = " + mDifference);
