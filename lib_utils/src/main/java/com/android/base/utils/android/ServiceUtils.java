...
/**
 * * @see <a href='https://github.com/Blankj/AndroidUtilCode/blob/master/lib/utilcode/src/main/java/com/blankj/utilcode/util/ServiceUtils.java'>AndroidUtilCode's ServiceUtils</a>
 */
public final class ServiceUtils {
    ...
    public static void startService(@NonNull final Class<?> cls) {
        try {
            Intent intent = new Intent(BaseUtils.getAppContext(), cls);
            intent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                BaseUtils.getAppContext().startForegroundService(intent);
            } else {
                BaseUtils.getAppContext().startService(intent);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static boolean stopService(@NonNull final Class<?> cls) {
        Intent intent = new Intent(BaseUtils.getAppContext(), cls);
        return BaseUtils.getAppContext().stopService(intent);
    }
     ...
     *                  </ul>
     */
    public static void bindService(@NonNull final String className, @NonNull final ServiceConnection conn, final int flags) {
        try {
     ...
     *              </ul>
     */
    public static void bindService(@NonNull final Class<?> cls, @NonNull final ServiceConnection conn, final int flags) {
        Intent intent = new Intent(BaseUtils.getAppContext(), cls);
        BaseUtils.getAppContext().bindService(intent, conn, flags);
    }
    ...
    public static void unbindService(@NonNull final ServiceConnection conn) {
        BaseUtils.getAppContext().unbindService(conn);
    }
